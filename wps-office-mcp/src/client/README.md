...
mac-poll-server.ts 对每个 pending requestId 只允许 /poll 派发一次，/status 会暴露 pending 派发状态；超时后晚到的 /result 会被识别为 late result 便于定位“执行成功但调用端超时”。proxy 模式转发到 58891 失败时会自动重置并尝试本地接管端口，避免旧 daemon 退出后状态仍停留在 proxy。
